...
1. Run from this repository's root unless the user identifies a different Git repository.
...
3. Write to `_temp/visual-diff.html` unless the user supplies an output path. `_temp/` is ignored by this project, so generated pages do not affect the Git worktree.
4. Run the bundled generator with the Python command available on the system:
   ...
   Replace `HEAD`, `WORKTREE`, and the output path with the requested values. Comparison points can be a Git commit-ish such as `HEAD~1`, a tag, a branch, or a commit SHA. Use `WORKTREE` for the current files.
...
`scripts/generate-split-view-diff.py` is a bundled standard-library-only generator. Its HTML is self-contained except for optional syntax-highlighting resources loaded by the page.
